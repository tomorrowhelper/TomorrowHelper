 ...
 */
public class HorizontalTwoChartView extends View {
   ...
   // private int color_line = Color.rgb(230, 230, 230);
     ...
     * 月份
     */
    private String month_num = "月月月月";


    public HorizontalTwoChartView(Context context) {
        super(context);
    }

    public HorizontalTwoChartView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public HorizontalTwoChartView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public HorizontalTwoChartView(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    protected void onDraw(Canvas canvas) {
       ...
       // int line_length = (int) (height - (fm.bottom - fm.top) - 4);
        ...
        for (int i = 0; i <= count; i++) {
            ...
            //底部百分比数字
            canvas.drawText(ratio_num, lift_width + i * line_width / count, height - 5, paint_font);
            // canvas.drawText(ratio_num, lift_width + i * line_width / 10, height - 10, paint_font);

            //        (168)
            // 162+0*842/5=162
            //网络线
           canvas.drawLine(lift_width + i * line_width / count, 0, lift_width + i * line_width / count, line_length+20, paint);
            /**
             *      //网络线
           ...
           //画最下面的横线
           if(i==count){
               canvas.drawLine(lift_width, line_length+20, (lin_width / count*i)+10, line_length+20, paint);
           }
            canvas.restore();
        }
        //获取月份文字信息
        ...
        for (int n = 0; n <ratio.length; n++) {
            canvas.save();
                                    /**
                                     *  ((line_length / 12)) * (13 - n) - line_length / 24 + (fm1.bottom - fm1.top) / 2
                                     line_length / 12 * (13 - n) - (line_length / 24 + plan_height / 2) + plan_height + fm1.bottom,
                                     */
            //左侧月份
            canvas.drawText(month_num, lift_width /  4 * 3+20, (line_length / count) * (n) - line_length / 24 + (fm1.bottom - fm1.top) / 2+40, paint_font2);
                                            //(line_length / ratio.length * (n) - (line_length / 24 + plan_height / 2) + plan_height + fm1.bottom)+40
            //比例图
            canvas.drawRect(lift_width,
                    (line_length / ratio.length * (n) - (line_length / 24 + plan_height / 2) + fm1.bottom)+40,
                    (line_width * (ratio[n] / count))+lift_width ,
